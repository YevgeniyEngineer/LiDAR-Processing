...
ObstacleClusterer::ObstacleClusterer(float neighbour_radius_threshold,
    ...
    : neighbour_radius_threshold_(neighbour_radius_threshold),
      cluster_quality_(cluster_quality), min_cluster_size_(min_cluster_size),
...
void ObstacleClusterer::clusterObstacles(
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZ>>& clustered_cloud)
{
    ...
    if (clustering_algorithm_ == ClusteringAlgorithm::DBSCAN)
    {
        ...
        using DBSCANPointCloud =
            clustering::DBSCANPointCloud<CoordinateType, 3>;
        using DBSCANClustering =
            clustering::DBSCANClustering<CoordinateType, 3>;

        auto point_cloud = std::make_unique<DBSCANPointCloud>();
        for (const auto& point : obstacle_cloud.points)
        {
            point_cloud->points.push_back(
                DBSCANPoint{point.x, point.y, point.z});
        }

        auto dbscan = std::make_unique<DBSCANClustering>(
            *point_cloud, neighbour_radius_threshold_, min_cluster_size_,
            max_cluster_size_);
             ...
             ClusteringAlgorithm::FAST_EUCLIDEAN_CLUSTERING)
    {
        ...
        auto fec_clustering = std::make_unique<FECClustering>(
            *point_cloud, neighbour_radius_threshold_, min_cluster_size_,
            max_cluster_size_, cluster_quality_);
