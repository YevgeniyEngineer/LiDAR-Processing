      ...
      initial_seed_threshold_(initial_seed_threshold)
{
    ...
    {
        throw std::runtime_error("Number of lowest point representative "
                                 "estimators must contain at least 3 points");
    }
                                           ...
                                           std::vector<pcl::PointCloud<pcl::PointXYZ>> &cloud_segments)
{
    ...
    // increasing x-order
    std::vector<std::size_t> sorted_indices(number_of_points);
...
GroundPlane estimatePlane(const Eigen::MatrixXf &points_xyz)
{
    ...
    // corresponds to normal vector to fitted plane See:
...
void GroundSegmenter::extractInitialSeeds(const pcl::PointCloud<pcl::PointXYZ> &cloud, BoundedDynamicArray &indices)
{
    ...
    // move indices
    indices.resize(upper_cutoff_index);
    for (int i = 0; i < upper_cutoff_index; ++i)
    {
        indices[i] = cloud_indices[i];
    }
                                     ...
                                     pcl::PointCloud<pcl::PointXYZ> &obstacle_cloud_segment)
{
    ...
    BoundedDynamicArray ground_cloud_indices;
    BoundedDynamicArray obstacle_cloud_indices;
    ...
    for (int iter_no = 0; iter_no < number_of_iterations_; ++iter_no)
    {
        ...
        Eigen::MatrixXf ground_points_xyz(number_of_ground_points,
                                          3); // N x 3
