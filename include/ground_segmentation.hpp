...
{
    float a, b, c, d;
    explicit GroundPlane(const float& a_, const float& b_, const float& c_,
                         const float& d_)
        : a(a_), b(b_), c(c_), d(d_){};
    ~GroundPlane() = default;
};
...
class GroundSegmenter final
{
  public:
    ...
    using BoundedDynamicArray =
        lidar_processing::utilities::BoundedDynamicArray<int, MAX_CLOUD_SIZE>;
    ...
    explicit GroundSegmenter(
        std::uint32_t number_of_iterations = 3,
        std::uint32_t number_of_planar_partitions = 2,
    ...
    void setNumberOfPlanarPartitions(
        const std::uint32_t& number_of_planar_partitions)
    {
  ...
  private:
    ...
    void extractInitialSeeds(const pcl::PointCloud<pcl::PointXYZ>& cloud,
                             BoundedDynamicArray& indices);
...
template <typename PointT>
void GroundSegmenter::segmentGround(
    const typename pcl::PointCloud<PointT>& input_cloud,
    pcl::PointCloud<pcl::PointXYZRGBL>& ground_cloud,
    pcl::PointCloud<pcl::PointXYZRGBL>& obstacle_cloud)
{
    ground_cloud.clear();
    ...
    {
        auto ground_cloud_segment =
            std::make_unique<pcl::PointCloud<pcl::PointXYZ>>();
        auto obstacle_cloud_segment =
            std::make_unique<pcl::PointCloud<pcl::PointXYZ>>();

        // segment points into ground and non-ground points
        fitGroundPlane(cloud_segment, *ground_cloud_segment,
                       *obstacle_cloud_segment);
    ...
    combineSegmentedPoints(ground_cloud_segments, SegmentationLabels::GROUND,
                           ground_cloud);
    combineSegmentedPoints(obstacle_cloud_segments,
                           SegmentationLabels::OBSTACLE, obstacle_cloud);
}
